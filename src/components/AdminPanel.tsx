                ...
                {filteredOrders.map(order => (
                  ...
                  >
                    ...
                    <div className="bg-amber-50/70 rounded-2xl p-3 border border-amber-200/80 space-y-2">
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2 text-xs flex-wrap">
                          ...
                          {order.paymentVerified ? (
                            ...
                            <span className="inline-flex items-center gap-1 text-[10px] font-black text-amber-800 bg-amber-100 border border-amber-300 px-2.5 py-0.5 rounded-full animate-pulse">
                      ...
                      {/* Receipt Proof Photo Row */}
                      {order.receiptImage ? (
                        <div className="flex items-center gap-3 pt-1 border-t border-amber-200/60">
                          <img
                            src={order.receiptImage}
                            alt="GCash Payment Proof"
                            className="w-14 h-14 object-cover rounded-xl border-2 border-amber-300 cursor-pointer shadow-xs hover:opacity-90 transition-all shrink-0"
                          ...
                          <div className="space-y-1">
                            <button
                              onClick={() => setZoomedReceipt(order.receiptImage!)}
                              className="px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-800 border border-blue-200 font-bold text-xs rounded-xl flex items-center gap-1.5 cursor-pointer transition-all shadow-2xs"
                            >
                              <Eye className="w-3.5 h-3.5 text-blue-600" />
                              <span>View Full GCash Receipt</span>
                            </button>
                            <span className="text-[10px] text-stone-500 font-medium block">
                              Click image thumbnail or button to inspect payment screenshot
                            </span>
                          </div>
                        </div>
                      ) : (
                        <div className="flex items-center gap-1.5 text-[11px] text-stone-500 italic pt-1 border-t border-amber-200/40">
                          <AlertCircle className="w-3 h-3 text-stone-400" />
                          <span>No receipt image attached (Cash on delivery / Over-the-counter)</span>
                        </div>
                      )}
                    </div>
