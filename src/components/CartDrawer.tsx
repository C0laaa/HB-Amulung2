  ...
  export default function CartDrawer({
    ...
    // Automatically populate Customer Name, Mobile Number, and Address from account, orders, or localStorage
    useEffect(() => {
      if (!isOpen) return;
      ...
      if (!customerName.trim()) {
        const foundName = customerAccount?.name || 
          initialCustomerName || 
          localStorage.getItem('honey_bakes_customer_name') || 
          (orders && orders.length > 0 ? orders[0].customerName : '');
        if (foundName) {
      ...
      // 2. Auto-fill Contact / Mobile Number
      if (!customerPhone.trim()) {
        const foundPhone = customerAccount?.phone || 
          localStorage.getItem('honey_bakes_customer_phone') || 
          (orders && orders.length > 0 ? orders.find(o => o.customerPhone)?.customerPhone : '');
        if (foundPhone) {
      ...
      // 3. Auto-fill Address for Delivery
      if (!address.trim()) {
        const foundAddress = localStorage.getItem('honey_bakes_customer_address') || 
          (orders && orders.length > 0 ? orders.find(o => o.address)?.address : '');
        if (foundAddress) {
          setAddress(foundAddress);
          const { km, matchedLandmark } = estimateDistanceKmFromAddress(foundAddress);
          setDistanceKm(km);
    ...
    }, [isOpen, customerAccount, initialCustomerName, orders]);

    const handleCustomerNameChange = (val: string) => {
      setCustomerName(val);
      if (val.trim()) {
        localStorage.setItem('honey_bakes_customer_name', val.trim());
      }
    };
    ...
    const handleCustomerPhoneChange = (val: string) => {
      ...
      if (val.trim()) {
        localStorage.setItem('honey_bakes_customer_phone', val.trim());
      }
    };
                ...
                {cart.length === 0 ? (
                  ...
                  <>
                    ...
                    <div className="bg-white p-4 rounded-2xl border border-brand-border shadow-sm space-y-3.5">
                      {isAutoFilled && (
                        <div className="bg-emerald-50 border border-emerald-200/80 px-3 py-1.5 rounded-xl text-[10.5px] font-bold text-emerald-900 flex items-center justify-between">
                          <span className="flex items-center gap-1.5">
                            <Sparkles className="w-3.5 h-3.5 text-emerald-600 fill-emerald-600 shrink-0" />
                            <span>Details auto-filled from your profile</span>
                          </span>
                          <span className="text-[9.5px] text-emerald-700 font-semibold uppercase tracking-wider">Saved</span>
                        </div>
                      )}
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (!receiptImage) {
                          ...
                          // Save details to localStorage for future auto-fills
                          localStorage.setItem('honey_bakes_customer_name', customerName.trim());
                          localStorage.setItem('honey_bakes_customer_phone', customerPhone.trim());
                          if (address.trim()) {
                            localStorage.setItem('honey_bakes_customer_address', address.trim());
                          }
